...
(function (root, factory) {
  ...
  /** 未提报原因
   *  说明：分发对象是 K800，货补基本官方全资，不涉及客户利润，
   *  所以不放「利润空间不足」，只保留「不愿破价（影响其他渠道价格体系）」。
   *  「入选链接非大链接」= 入池的是小链接、主推大链接没入池，与原「主链接不在池」重复，故合并为一项。
   */
  ...
  /** 选中后需要额外补充说明的选项：key=选项，value=[标题, 占位提示] */
  var NO_REPORT_NEED_DETAIL = {
    '入选链接非主推大链接': [
      '为什么主推大链接不能提报',
      '如：小链接专供私域等特殊渠道，不做公域投放 / 小链接的 SKU 货组与直播间不一致 / 大链接未入池',
    ],
    '其他': ['其他原因', '请具体描述'],
  };
  ...
  function newProduct(p) {
    ...
    return {
      ...
      planTime: '',          // 择机报 → 预计提报时间
      reasons: [],           // 未提报 → 原因（多选）
      smallLinkReason: '',   // 未提报·入选链接非主推大链接 → 原因说明
      reasonOther: '',
      // 无提报资格（商品维度）
      unqualifiedReasons: [],
      shopScore: '',
